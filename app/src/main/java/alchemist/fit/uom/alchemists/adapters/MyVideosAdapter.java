...
public class MyVideosAdapter extends AAH_VideosAdapter {
    ...
    private final List<FileUploadInfo> list;
    private final Picasso picasso;
    private static final int TYPE_VIDEO = 0, TYPE_TEXT = 1;
    private String previewImage ="http://adstriangle.com/triangle/wp-content/uploads/2017/04/loading-time-300x150.jpg";
    ...
    public class MyViewHolder extends AAH_CustomViewHolder {
        ...
        public MyViewHolder(View x) {
            super(x);
            tv = ButterKnife.findById(x, R.id.ImageNameTextView);
    ...
    public MyVideosAdapter(List<FileUploadInfo> TempList, Picasso p) {
        this.list = TempList;
        this.picasso = p;
    }
    ...
    public AAH_CustomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.recyclerview_items2, parent, false);
    ...
    public void onBindViewHolder(final AAH_CustomViewHolder holder, int position) {
        FileUploadInfo UploadInfo = list.get(position);
        ((MyViewHolder) holder).tv.setText(UploadInfo.getFileName());
        if (UploadInfo.getFileType().equals("image")) {
            holder.setImageUrl(UploadInfo.getFileURL());
        } else {
            holder.setImageUrl(previewImage);
            holder.setVideoUrl(UploadInfo.getFileURL());
        }

        if (UploadInfo.getFileType().equals("image")) {
            //load image into imageview
            if (UploadInfo.getFileURL() != null && !UploadInfo.getFileURL().isEmpty()) {
                picasso.load(holder.getImageUrl()).config(Bitmap.Config.RGB_565).into(holder.getAAH_ImageView());
            }

        }

        if (UploadInfo.getFileType().equals("video")) {
            //load image into imageview
